...
import { printTaggedReceiveVoucher } from '../../utils/approvalVoucherPrint';
...
export default function ApprovalReceivePage() {
  ...
  const tourSteps = [
    { title: '1. Find the Voucher', description: 'Type or scan the Approval Issue voucher number here to pull up every item still pending against it.', target: () => searchCardRef.current },
    { title: '2. Select What Came Back', description: 'Tick the items the customer is returning or has bought. Anything left unticked stays "Pending" on this voucher for a later visit.', target: () => tableCardRef.current },
  ...
  const searchMutation = useMutation({
    mutationFn: (v) => approvalApi.getIssueByVoucher(v),
    onSuccess: (res) => {
  ...
  const receiveMutation = useMutation({
    mutationFn: (data) => approvalApi.createReceive(data),
    onSuccess: (res) => {
  ...
  const selectedItems = pendingItems.filter(i => selectedRowKeys.includes(i.Issue_Item_ID));
  ...
  const submitReceive = () => {
    ...
    receiveMutation.mutate({
      Issue_ID: issue.Issue_ID, Receive_Date: receiveDate.format('YYYY-MM-DD'),
      Remarks: remarks || null, issueItemIds: selectedRowKeys,
    });
  };

  const columns = [
    { title: 'Barcode', dataIndex: 'Article_Number', render: v => <Text code style={{ fontSize: 11 }}>{v}</Text> },
    ...
    { title: 'Net Wt', dataIndex: 'Net_Gold_Weight', render: formatWeight },
  ...
  return (
    <div className="page-wrapper">
      <div className="page-header">
        <Title level={4} style={{ margin: 0 }}><UndoOutlined style={{ color: '#B8860B', marginRight: 8 }} />Approval Receive</Title>
      </div>
      ...
      <Card size="small" style={{ borderRadius: 8, marginBottom: 14 }}>
        <Space.Compact style={{ width: '100%' }}>
          <Input size="large" placeholder="Enter Approval Issue Voucher Number (e.g. APR-ISS-TENANT-20260719-0001)"
          ...
          <Col xs={24} lg={16}>
            ...
            <Card size="small"
              ...
              {pendingItems.length === 0
                ...
                : <Table scroll={{ x: 'max-content' }} columns={columns} dataSource={pendingItems} rowKey="Issue_Item_ID" size="small" pagination={false}
        ...
        {createdReceive && (
          <>
            ...
            <Space style={{ width: '100%', marginTop: 20 }}>
              <Button icon={<PrinterOutlined />} onClick={() => printTaggedReceiveVoucher(createdReceive.receive, createdReceive.issue, createdReceive.items)}>
                Print Voucher
              </Button>
