...
afterAll(async () => {
  for (const id of createdTenantIds) {
    await db('tbl_invoice_template_master').where({ Tenant_ID: id }).del();
    await db('tbl_customer_display_settings').where({ Tenant_ID: id }).del();
    await db('tbl_tenant_modules').where({ Tenant_ID: id }).del();
    await db('tbl_user_master').where({ Tenant_ID: id }).del();
    await db('tbl_branch_master').where({ Tenant_ID: id }).del();
...
test('regression: creating a SECOND tenant right after a first does not collide on cloned template Sync_UUID', async () => {
  ...
  const firstTemplates = await db('tbl_invoice_template_master').where({ Tenant_ID: 'QATCB1' }).select('Sync_UUID');
  const secondTemplates = await db('tbl_invoice_template_master').where({ Tenant_ID: 'QATCB2' }).select('Sync_UUID');
